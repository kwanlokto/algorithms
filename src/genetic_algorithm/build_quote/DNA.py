...
class DNA(BaseDNA):
    ...
    def cross_over(cls, parents: list, target):
        ...
        for p_idx in range(len(parents)):
            ...
            new_traits = [
                traits[t_idx]
                for t_idx in range(per_parent * p_idx, len(traits))
                if t_idx < min(per_parent * (p_idx + 1), len(traits))
            ]
            # TODO: make sure that the parents' traits are in the same order or
            # this will cause issues. OR find a way to bypass that
            child_traits = [*child_traits, *new_traits]
